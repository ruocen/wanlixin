...
namespace WLX.Utils.WebUtils
{
    ...
    public class XMLHelper
    {
        ...
        {
            return CreateXmlDocument(xmlFileName, rootNodeName, "", version, encoding, standalone);
        }
        ...
        ///<param name="xmlFileName">XML文档完全文件名(包含物理路径)</param>
        ///<param name="rootNodeName">XML文档根节点名称(须指定一个根节点名称)</param>
        ///<param name="childNodeName">XML文档根节点名称的一个子节点</param>
        ///<param name="version">XML文档版本号(必须为:"1.0")</param>
        ///<param name="encoding">XML文档编码方式</param>
        ///<param name="standalone">该值必须是"yes"或"no",如果为null,Save方法不在XML声明上写出独立属性</param>
        ///<returns>成功返回true,失败返回false</returns>
        public static bool CreateXmlDocument(string xmlFileName, string rootNodeName,string childNodeName, string version, string encoding, string standalone)
        {
            ...
            try
            {
                if (!File.Exists(xmlFileName))
                {
                    ...
                    {
                        XmlElement xmlC = xmlDoc.CreateElement(childNodeName);
                        xmlC.SetAttribute("id", "");
                        root.AppendChild(xmlC);
                    }
